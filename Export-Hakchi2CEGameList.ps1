...
.Notes
	...
	Version       : 1.1 - 2/5/2022 - Add CSV output
...
Foreach ($Game in $HakchiConfig.gamesCollectionSettings.$ConsoleTypeName.SelectedGames)
{
    ...
    {
        $GameInfo = New-Object PSObject
        $data = Get-Content -Raw -Path .\games\$Game\metadata.json | ConvertFrom-Json
        $GameInfo | Add-Member -MemberType NoteProperty -Name "Filename" -Value $data.OriginalFilename
        if ($data.System)
        {
            $GameInfo | Add-Member -MemberType NoteProperty -Name "System" -Value $data.System
        }
        else
        {
            $GameInfo | Add-Member -MemberType NoteProperty -Name "System" -Value $data.Core
        }
        $data = Get-Content -Path .\games\$Game\$Game.desktop
        $GameInfo | Add-Member -MemberType NoteProperty -Name "Name" -Value $data[4].Substring($data[4].IndexOf("=")+1)
        $GameList += $GameInfo
    }
}
Foreach ($Game in $HakchiConfig.gamesCollectionSettings.$ConsoleTypeName.OriginalGames)
{
    if (Test-Path -Path .\games_originals\$Game\$Game.desktop)
    {
        ...
        $GameInfo | Add-Member -MemberType NoteProperty -Name "Name" -Value $data[4].Substring($data[4].IndexOf("=")+1)
        $GameInfo | Add-Member -MemberType NoteProperty -Name "Filename" -Value "Original"
        $GameInfo | Add-Member -MemberType NoteProperty -Name "System" -Value $ConsoleTypeName
        $GameList += $GameInfo
    }
}
$GameList = $GameList | Sort-Object -Property Name
...
window.onload=function(){new TableSort("gamelist");var rows=document.getElementById("gamelist").rows.length-1;document.getElementById("total").innerHTML=rows;};
</script>
</head>
<body>
...
{
    $HTML += "<tr><td>"+$Game.Name+"</td><td>"+$Game.System+"</td><td>"+$Game.Filename+"</td></tr>`n"
}
...
#Generate CSV File
$GameList | select Name,System,Filename | Export-Csv -Path '.\ClassicMiniGameList.csv' -NoTypeInformation
